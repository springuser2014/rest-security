...
public class CookieAuthenticationFilter extends GenericFilterBean {
	...
	/**
	 * Loads information such as roles etc about the user if the user cookie
	 * were present. This is a great place to introduce a cache if required to
	 * prevent hitting ldap on every call.
	 * 
	 * @param request
	 *            HttpServletRequest
	...
	private SecurityContext loadSecurityContext(HttpServletRequest request) {
		...
		return userName != null ? new CookieSecurityContext(userDetailService.loadUserByUsername(userName)) : SecurityContextHolder.createEmptyContext();
	}
}
